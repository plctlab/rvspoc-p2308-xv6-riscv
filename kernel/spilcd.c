...
#define CVI_PWRGPIO_BASE 0x05021000
...
int
spilcdread(int user_dst, uint64 dst, int n)
{
  return -1;
}

int
spilcdwrite(int user_src, uint64 src, int n)
{
  struct spilcd_ctrl value;
  uint32_t tmp;
  ...
  switch (value.command) {
    case SPILCD_CMD_RESET:
	    tmp = mmio_read_32(CVI_PWRGPIO_BASE);
      if (value.u.data == 0) {
        /* gpio RESET = 0 */
        tmp &= ~(1 << 21);
      } else {
      ...
      mmio_write_32(CVI_PWRGPIO_BASE, tmp);
      break;

    case SPILCD_CMD_WRITE_REG:
      /* clear DC */
      tmp = mmio_read_32(CVI_PWRGPIO_BASE);
      tmp &= ~(1 << 18);
      mmio_write_32(CVI_PWRGPIO_BASE, tmp);
      ...
      /* clear cs */
      tmp = mmio_read_32(CVI_GPIOC_BASE);
      tmp &= ~(1 << 9);
      mmio_write_32(CVI_GPIOC_BASE, tmp);

      /* write data */
      writel(value.u.data & 0xFF, (volatile uint8_t *)CVI_SPI2_BASE + DW_SPI_DR);
...
spilcdinit(void)
{
  ...
  kvmmap(kernel_pagetable, CVI_PWRGPIO_BASE, CVI_PWRGPIO_BASE, PGSIZE, PTE_R | PTE_W | PMA_DEVICE);

  /* set pinmux */
  PINMUX_CONFIG(SD1_CMD, SPI2_SDO); // SPI MOSI
  PINMUX_CONFIG(SD1_CLK, SPI2_SCK); // SPI CLK

  PINMUX_CONFIG(SD1_D0, PWR_GPIO_21); // RESET
  PINMUX_CONFIG(SD1_D3, PWR_GPIO_18); // DC
  PINMUX_CONFIG(PAD_MIPIRX1P, XGPIOC_9); // CS

  /* init gpio output function */
  tmp = mmio_read_32(CVI_PWRGPIO_BASE + 4);
	mmio_write_32(CVI_PWRGPIO_BASE + 4, tmp | (1 << 21) | (1 << 18) | (1 << 22) | (1<< 23));

  tmp = mmio_read_32(CVI_GPIOC_BASE + 4);
	mmio_write_32(CVI_GPIOC_BASE + 4, tmp | (1 << 9));
